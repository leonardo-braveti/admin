    ...
    $.glisse = function (element, options) {
        ...
        var createElements = function createElements() {
            ...
            plugin.els['content'].on('click', function() {  
                ...
                /************* codigo para excluir imagem **********/
                decisao = confirm("Deseja realmente excluir esta imagem?"); 
                if(decisao){
                    caminho = $(this).css("background-image").split("/");
                    tamanho = caminho.length;
                    quase = caminho[tamanho-1].split("\"");
                    // quase[0] é o nome da imagem

                    $.ajax({
                        //url: "http://localhost/lotus/admin/index.php/fotos/excluir_bynome/"+quase[0], // url para rodar no localhost
			url: "http://www.braveti.com.br/lotus/admin/index.php/fotos/excluir_bynome/"+quase[0], // url para rodar no localhost
                        success:function(data){
                            // refresh na tela para recarregar as fotos remanescentes 
                            //$(location).attr('href','http://localhost/lotus/admin/index.php/fotos');
			    $(location).attr('href','http://www.braveti.com.br/lotus/admin/index.php/fotos');
                        },
                        error:function(data){
                            alert("Desculpe, a foto não foi excluída");
                        }                       
                    });
